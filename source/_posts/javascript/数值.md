...
前面说过，Javascript 的 64 位浮点数之中，有一个二进制位是符号位。这意味着，任何一个数都有一个对应的值，就连 `0` 也不例外。
...
上面的代码运行时，会自动将字符串 `x` 转为数值，但是由于 `x` 不是数值，所以最后得到结果为 `NaN`，表示它是**非数字**(`NaN`)。
...
`Infinity` 大于一切数值（别无他物了`NaN`），`-Infinity`小于一切数值（除了 `NaN`）。
